...
class Login extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        // if ($this->session->userdata('admin_login') != 1 && $this->session->userdata('mod_login') != 1) {
    ...
    public function index() {
        ...
        // Nếu là Mod thì vào trang Mod (nếu bạn đã thêm logic này)
        elseif ($this->session->userdata('mod_login')) {
            redirect(site_url('mod/dashboard'), 'refresh');
        }
        // Nếu là User (Học viên) thì về Trang chủ
        elseif ($this->session->userdata('user_login')) {
            redirect(site_url('home'), 'refresh'); // <--- Sửa dòng này
        } 
    ...
    public function validate_login($from = "") {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                $this->session->set_userdata('mod_login', '1'); // Gắn nhãn Mod login
                $this->session->set_userdata('role', 'mod');    // Để view index.php nhận diện
                redirect(site_url('mod/dashboard'), 'refresh');
            }
        }else {
    ...
    public function session_destroy() {
        ...
        $this->session->unset_userdata('mod_login');
        $this->session->unset_userdata('admin_login'); // Xóa hết cho chắc
